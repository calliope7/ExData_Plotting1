...
# Top right
with(householdPowerConsumption, plot(Time,Voltage, xlab="datetime", type="n"))
with(householdPowerConsumption, lines(Time, Voltage, col="black"))

# old plot 3 (Bottom left)
...
legend("topright", lty=1, col=c("black", "red", "blue"), legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"), inset=0.01, box.lty=0)

# Bottom right
with(householdPowerConsumption, plot(Time, Global_reactive_power, xlab="datetime", type="n"))
with(householdPowerConsumption, lines(Time, Global_reactive_power, col="black"))
